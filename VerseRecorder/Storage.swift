...
class RecordingStorage {
    ...
    func getRecordings() -> [RangeRecording] {
        
        if let savedRecording = Storage.shared.retrieve(forKey: "recordings") as? Data {
            let decoder = JSONDecoder()
            if let loadedRecording = try? decoder.decode([RangeRecording].self, from: savedRecording) {
    ...
    func addRecording(_ recording: RangeRecording) {
    ...
    func deleteRecording(_ recording: RangeRecording) {
        var recordings = getRecordings()
